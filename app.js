document.addEventListener("DOMContentLoaded", () => {
  ...
  // --- Page load animation ---
  const LOADER_MIN_MS = 760;
  const LOADER_FAILSAFE_MS = 4600;
  const LOADER_HIDE_MS = 420;
  const LOADER_ID = "siteLoader";
  let loaderBootAt = Date.now();
  ...
  const ensureSiteLoader = () => {
    ...
    loader.innerHTML =
      `<div class="site-loader__card">` +
      `<div class="site-loader__top">` +
      `<div class="site-loader__orb"></div>` +
      `<div class="site-loader__title">NeonPuff</div>` +
      `</div>` +
      `<div class="site-loader__line"></div>` +
      `<div class="site-loader__meta">загрузка...</div>` +
      `</div>`;
  ...
  const bootSiteLoader = () => {
    ...
    const scheduleFinish = () => {
      if (scheduled) return;
      scheduled = true;
      const elapsed = Date.now() - loaderBootAt;
      const wait = Math.max(0, minDelay - elapsed);
      window.setTimeout(finishSiteLoader, wait);
    };

    if (document.readyState === "complete") scheduleFinish();
    else window.addEventListener("load", scheduleFinish, { once: true });

    window.setTimeout(scheduleFinish, LOADER_FAILSAFE_MS);
  };
  ...
  const animateStagger = (root, selector) => {
    if (!root) return;
  ...
  const renderFeatured = () => {
    ...
    animateStagger(featuredGrid, ".product-card");
  };
  ...
  const renderShop = () => {
    ...
    animateStagger(productsGrid, ".product-card");
  };
  ...
  const renderProduct = () => {
    ...
    animateStagger(productRoot, ".product");
    animateStagger(productRoot, ".spec");
  };
  ...
  const renderCartPage = () => {
    ...
    cartPageItems.innerHTML = items
      ...
      .join("");
    animateStagger(cartPageItems, ".cart-item");
  };
